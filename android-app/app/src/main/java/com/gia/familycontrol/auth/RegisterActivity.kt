...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun doRegister() {
        ...
        lifecycleScope.launch {
            try {
                val response = api.register(RegisterRequest(email, password, name, "CHILD"))
                if (response.isSuccessful && response.body() != null) {
                    ...
                    getSharedPreferences("gia_prefs", MODE_PRIVATE).edit()
                        ...
                        .apply()
                    startActivity(Intent(this@RegisterActivity, ChildLauncherActivity::class.java))
                    finishAffinity()
                } else {
                    setLoading(false)
                    showError(if (response.code() == 400) "Email already registered" else "Registration failed. Try again.")
                }
            } catch (e: Exception) {
                setLoading(false)
